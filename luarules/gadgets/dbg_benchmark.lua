...
function gadget:GetInfo()
	return {
		...
		layer = -1999999999,
		enabled = (Spring.GetModOptions and (Spring.GetModOptions().allow_dev_features == true or Spring.GetModOptions().allow_dev_features == "1" or Spring.GetModOptions().allow_dev_features == 1)) or false,
	}
end
...
if gadgetHandler:IsSyncedCode() then
	...
	function benchmark(words)
		...
		if not benchmarkenabled then
			Spring.Echo(
				string.format(
					"Benchmark ended, %d units spawned over %d gameframes, Units/frame = %f",
		...
		Spring.Echo(
			string.format(
				"Starting benchmark %s vs %s with %i maxunits and %i units per step in a %d radius, features live %d frames",
				team1unitDefName,
				team2unitDefName,
				maxunits,
				feedstep,
	...
	function benchmark(_, line, words, playerID, action)
		...
		if benchmarkactive then
			...
			local s1 = string.format(
				"Benchmark complete, #created = %d, #destroyed = %d",
				benchmarkstats.numunitscreated,
				benchmarkstats.numunitsdestroyed
			)
			Spring.Echo(s1)
			local res = {}
			local stats = {}
			for n, t in pairs({
				Sim = benchmarkstats.simFrameTimes,
				Draw = benchmarkstats.drawFrameTimes,
