...
function updateInputDlist()
	...
	textInputDlist = glCreateList(function()
		...
		local x2 = math.max(
			...
			floor(activationArea[1] + ((activationArea[3] - activationArea[1]) / 5))
		)
		chatInputArea = {
			activationArea[1],
		...
		if group.numOptions > 0 then
			...
			if #grOptions > 0 then
				...
				newOptions[newOptionsCount] =
					{ id = "group_" .. group.id, name = "\255\255\200\110" .. name, type = "label" }
			end
...
function mouseoverGroupTab(id)
	...
	RectRound(
		groupRect[id][1] + groupMargin,
		groupRect[id][2],
		groupRect[id][3] - groupMargin,
		groupRect[id][4] - groupMargin,
		groupMargin * 1.8,
		1,
		1,
		0,
	...
	RectRound(
		groupRect[id][1] + groupMargin,
		groupRect[id][4] - groupMargin - ((groupRect[id][4] - groupRect[id][2]) * 0.5),
		groupRect[id][3] - groupMargin,
		groupRect[id][4] - groupMargin,
		groupMargin * 1.8,
		1,
	...
	RectRound(
		...
		groupRect[id][2],
		groupRect[id][3] - groupMargin,
		groupRect[id][2] + groupMargin + ((groupRect[id][4] - groupRect[id][2]) * 0.5),
		groupMargin * 1.25,
		0,
		0,
		0,
	...
	font2:Print(
		...
		groupRect[id][1] + ((groupRect[id][3] - groupRect[id][1]) / 2),
		screenY + (9 * widgetScale),
		tabFontSize,
		"con"
	)
	font2:End()
...
function DrawWindow()
	...
	UiElement(
		...
		screenX + screenWidth,
		screenY,
		(showTextInput and inputText ~= "" and inputMode == "") and 1 or 0,
		0,
		1,
		(showTextInput and 0 or 1),
		1,
		1,
		1,
		1,
	...
	titleRect = {
		math.floor((screenX + screenWidth) - ((font2:GetTextWidth(title) * titleFontSize) + (titleFontSize * 1.5))),
		screenY,
		screenX + screenWidth,
	...
	RectRound(
		...
		0,
		0,
		WG.guishader and { 0, 0, 0, 0.8 } or { 0, 0, 0, 0.85 },
		WG.guishader and { 0.05, 0.05, 0.05, 0.8 } or { 0.05, 0.05, 0.05, 0.85 }
	)
	RectRound(
		titleRect[1] + groupMargin,
		titleRect[4] - groupMargin - ((titleRect[4] - titleRect[2]) * 0.5),
		titleRect[3] - groupMargin,
		titleRect[4] - groupMargin,
		elementCorner * 0.66,
		1,
		1,
		0,
		...
		for id, group in pairs(optionGroups) do
			if group.numOptions > 0 then
				groupRect[id] = {
					xpos,
					titleRect[2],
					math.floor(xpos + (font2:GetTextWidth(group.name) * tabFontSize) + (33 * widgetScale)),
					titleRect[4],
				}
				if devMode or devUI or group.id ~= "dev" then
					...
					if currentGroupTab == nil or currentGroupTab ~= group.id then
						...
						RectRound(
							groupRect[id][1] + groupMargin + groupPadding,
							groupRect[id][2],
							groupRect[id][3] - groupMargin - groupPadding,
							groupRect[id][4] - groupMargin - groupPadding,
							elementCorner * 0.5,
							1,
							1,
							0,
	...
	if changesRequireRestart then
		...
		RectRound(
			screenX + bgpadding,
			screenY - screenHeight + bgpadding,
			screenX + screenWidth - bgpadding,
			screenY - screenHeight + (31 * widgetScale),
			elementCorner,
			0,
			0,
			1,
		...
		RectRound(
			screenX + bgpadding,
			screenY - screenHeight + bgpadding + (30 * widgetScale) - 1,
			screenX + screenWidth - bgpadding,
			screenY - screenHeight + (30 * widgetScale),
			0,
			0,
			0,
			0,
		...
		font:SetOutlineColor(0, 0, 0, 0.4)
		font:Print(
			BAR.I18N("ui.settings.madechanges"),
			screenX + math.floor(screenWidth * 0.5),
			screenY - screenHeight + (12 * widgetScale),
			15 * widgetScale,
			"cn"
		)
	end
	...
	if totalColumns > maxShownColumns then
		...
		if (startColumn - 1) + maxShownColumns < totalColumns then
			optionButtonForward =
				{ startX - buttonSize - buttonMarginX, startY, startX - buttonMarginX, startY + buttonSize }
			glColor(1, 1, 1, 1)
		...
		font:Print(
			math.ceil(startColumn / maxShownColumns) .. " / " .. math.ceil(totalColumns / maxShownColumns),
			startX - (buttonSize * 2.6) - buttonMarginX,
			startY + buttonSize / 2.6,
			buttonSize / 2.4,
			"rn"
		)
		if startColumn > 1 then
			optionButtonBackward = {
				startX - (buttonSize * 2) - buttonMarginX - (buttonMarginX / 1.5),
				startY,
				startX - (buttonSize * 1) - buttonMarginX - (buttonMarginX / 1.5),
				startY + buttonSize,
			}
			glColor(1, 1, 1, 1)
			glTexture(backwardTex)
			glTexRect(
				optionButtonBackward[1],
				optionButtonBackward[2],
				optionButtonBackward[3],
				optionButtonBackward[4]
			)
			glTexture(false)
				...
				if column >= startColumn then
					...
					if option.name then
						...
						if option.type == nil then
							...
							font3:Print(
								"\255\255\200\130" .. option.name,
								xPos + (oPadding * 0.5),
								yPos - (oHeight * 1.8) - oPadding,
								oHeight * 1.5,
								"no"
							)
							font3:End()
							...
							if width > maxWidth then
								...
								elseif
									option.description ~= option.name
									and string.sub(option.description, 1, string.len(option.name)) ~= option.name
								then
						...
						optionHover[oid] = {
							math.floor(xPos),
							math.floor(yPos - oHeight - oPadding),
							math.floor(xPosMax),
							math.floor(yPos + oPadding),
						}
						...
						if option.type == "click" then
							optionButtons[oid] = {}
							optionButtons[oid] = {
								math.floor(xPos + rightPadding),
								math.floor(yPos - oHeight),
								math.floor(xPosMax - rightPadding),
								math.floor(yPos),
							}
						elseif option.type == "bool" then
							optionButtons[oid] = {}
							optionButtons[oid] = {
								math.floor(xPosMax - boolWidth - rightPadding),
								math.floor(yPos - oHeight),
								math.floor(xPosMax - rightPadding),
								math.floor(yPos),
							}
						...
						elseif option.type == "slider" then
							if type(option.value) == "number" then -- just to be safe
								...
								if type(sliderPos) == "number" then
									UiSlider(
										math.floor(xPosMax - (sliderSize / 2) - sliderWidth - rightPadding),
										math.floor(yPos - ((oHeight / 7) * 4.5)),
										math.floor(xPosMax - (sliderSize / 2) - rightPadding),
										math.floor(yPos - ((oHeight / 7) * 2.8)),
										option.steps and option.steps or option.step,
										option.min,
										option.max
									)
									UiSliderKnob(
										math.floor(
											xPosMax
												- (sliderSize / 2)
												- sliderWidth
												+ (sliderWidth * sliderPos)
												- rightPadding
										),
										math.floor(yPos - oHeight + (oHeight / 2)),
										math.floor(sliderSize / 2)
									)
									optionButtons[oid] = {
										xPosMax
											- (sliderSize / 2)
									...
									optionButtons[oid].sliderXpos = {
										xPosMax - (sliderSize / 2) - sliderWidth - rightPadding,
										xPosMax - (sliderSize / 2) - rightPadding,
									}
								end
							end
						elseif option.type == "select" then
							optionButtons[oid] = {
								math.floor(xPosMax - selectWidth - rightPadding),
								math.floor(yPos - oHeight),
								math.floor(xPosMax - rightPadding),
								math.floor(yPos),
							}
							...
							if option.options[tonumber(option.value)] ~= nil then
								...
								if
									font:GetTextWidth(text) * math.floor(15 * widgetScale)
									> (optionButtons[oid][3] - optionButtons[oid][1]) * 0.93
								then
									while
										font:GetTextWidth(text) * math.floor(15 * widgetScale)
										> (optionButtons[oid][3] - optionButtons[oid][1]) * 0.9
									do
										text = string.sub(text, 1, string.len(text) - 1)
									end
									text = text .. "..."
									...
									elseif
										option.description ~= option.name
										and string.sub(option.description, 1, string.len(option.name))
											~= option.name
									then
										option.description = option.name .. "\n\255\255\255\255" .. option.description
									end
								...
								if option.id == "font2" then
									...
									font2:Print(
										text,
										xPosMax - selectWidth + 5 - rightPadding,
										yPos - (fontSize / 2) - oPadding,
										fontSize,
										"no"
									)
									font2:End()
...
function widget:Update(dt)
			...
			if isOffscreenTime then
				if isOffscreenTime + muteFadeTime > now then
					if isOffscreen then
						Spring.SetConfigInt(
							"snd_volmaster",
							prevOffscreenVolume * (1 - ((now - isOffscreenTime) / muteFadeTime))
						)
					else
						Spring.SetConfigInt(
							"snd_volmaster",
							prevOffscreenVolume * ((now - isOffscreenTime) / muteFadeTime)
						)
					end
				else
					isOffscreenTime = nil
	...
	if sec2 > 0.5 then
		...
		if
			apiUnitTrackerEnabledCount < 10
			and widgetHandler.orderList["API Unit Tracker DEVMODE GL4"]
			and widgetHandler.orderList["API Unit Tracker DEVMODE GL4"] < 0.5
		then
	...
	if show and (sec > lastUpdate + 0.5 or forceUpdate) then
		...
		for i, option in ipairs(unfilteredOptions) do
			if
				...
				and option.value ~= GetWidgetToggleValue(option.widget)
			then
	...
	if show then
		...
		elseif groupRect ~= nil then
			for id, group in pairs(optionGroups) do
				if devMode or devUI or group.id ~= "dev" then
					if
						math_isInRect(mx, my, groupRect[id][1], groupRect[id][2], groupRect[id][3], groupRect[id][4])
					then
						return true
					end
				end
...
function widget:DrawScreen()
	...
	if not initialized then
		...
		if (show or showOnceMore) and windowList then
			...
			local mx, my, ml = Spring.GetMouseState()
			if
				(math_isInRect(mx, my, windowRect[1], windowRect[2], windowRect[3], windowRect[4]))
				or (titleRect and math_isInRect(mx, my, titleRect[1], titleRect[2], titleRect[3], titleRect[4]))
				or (
					chatInputArea
					and math_isInRect(mx, my, chatInputArea[1], chatInputArea[2], chatInputArea[3], chatInputArea[4])
				)
			then
			...
			if
				...
				and titleRect ~= nil
				and math_isInRect(mx, my, titleRect[1], titleRect[2], titleRect[3], titleRect[4])
			then
				local groupMargin = math.floor(bgpadding * 0.8)
				-- gloss
				glBlending(GL_SRC_ALPHA, GL_ONE)
			...
			if
				...
				and math_isInRect(
					mx,
					my,
					optionButtonForward[1],
					optionButtonForward[2],
					optionButtonForward[3],
					optionButtonForward[4]
				)
			then
				RectRound(
					optionButtonForward[1],
					...
					optionButtonForward[3],
					optionButtonForward[4],
					(optionButtonForward[4] - optionButtonForward[2]) / 12,
					2,
					2,
					2,
					2,
					ml and { 1, 0.91, 0.66, 0.1 } or { 1, 0.91, 0.66, 0.3 },
					{ 1, 0.91, 0.66, 0.2 }
				)
			end
			if
			...
			then
				RectRound(
					...
					optionButtonBackward[3],
					optionButtonBackward[4],
					(optionButtonBackward[4] - optionButtonBackward[2]) / 12,
					2,
					2,
					2,
					2,
			...
			if not showSelectOptions then
				...
				if not tooltipShowing then
					for i, o in pairs(optionHover) do
						if
							options[i]
							and math_isInRect(mx, my, o[1], o[2], o[3], o[4])
							and options[i].type
							and options[i].type ~= "label"
							and options[i].type ~= "text"
						then
							-- display console command at the bottom
							if
								(advSettings or devMode or devUI) and (options[i].onchange ~= nil or options[i].widget)
							then
								if
									not consoleCmdDlist
									or not lastConsoleCmdOption
									or lastConsoleCmdOption ~= options[i].id
								then
									...
									consoleCmdDlist = glCreateList(function()
										...
										font:Print(
											"/option " .. options[i].id,
											screenX + (8 * widgetScale),
											screenY - screenHeight + (11 * widgetScale),
											14 * widgetScale,
											"n"
										)
										font:End()
							...
							UiSelectHighlight(
								...
								o[4],
								nil,
								options[i].onclick and (ml and 0.35 or 0.22) or 0.14,
								options[i].onclick and { 0.5, 1, 0.25 }
							)
							if
								WG.tooltip
								and options[i].description
								and options[i].description ~= ""
								and options[i].description ~= " "
							then
								...
								if options[i].restart then
									desc = desc
										.. "\n\n\255\255\120\120"
										.. BAR.I18N("ui.settings.changesrequirerestart")
								end
								local showTooltip = true
								if options[i].nametext and string.find(options[i].nametext, desc, nil, true) then
									if
										string.len(desc)
										== (string.len(options[i].nametext) + 1)
											- string.find(options[i].nametext, desc, nil, true)
									then
										showTooltip = false
									end
								end
			...
			if showSelectOptions ~= nil then
				...
				if options[showSelectOptions].id == "preset" then
					for optionID, _ in pairs(presets.lowest) do
						...
						if optionHover[optionKey] ~= nil then
							RectRound(
								optionHover[optionKey][1],
								optionHover[optionKey][2] + 1,
								optionHover[optionKey][3],
								optionHover[optionKey][4] - 1,
								1,
								2,
								2,
								2,
				...
				selectOptionsList = glCreateList(function()
					...
					RectRound(
						optionButtons[showSelectOptions][1] - borderSize,
						yPos - oHeight - oPadding - borderSize,
						optionButtons[showSelectOptions][1] + maxWidth + borderSize,
						optionButtons[showSelectOptions][2] + borderSize,
						(optionButtons[showSelectOptions][4] - optionButtons[showSelectOptions][2]) * 0.1,
						1,
						1,
						1,
						1,
					...
					RectRound(
						...
						1,
						1,
						{ 0.3, 0.3, 0.3, WG.guishader and 0.84 or 0.94 },
						{ 0.35, 0.35, 0.35, WG.guishader and 0.84 or 0.94 }
					)
					UiSelector(
						optionButtons[showSelectOptions][1],
						optionButtons[showSelectOptions][2],
					...
					for k, option in pairs(options[showSelectOptions].options) do
						...
						if i > selectScrollOffset and vi < maxVisible then
							...
							optionSelect[#optionSelect + 1] = {
								math.floor(optionButtons[showSelectOptions][1]),
								math.floor(itemYPos - oHeight - oPadding),
								math.floor(optionButtons[showSelectOptions][1] + maxWidth),
								math.floor(itemYPos + oPadding) - 1,
								k,
							}
							...
							if
								math_isInRect(
									...
									my,
									optionSelect[#optionSelect][1],
									optionSelect[#optionSelect][2],
									optionSelect[#optionSelect][3],
									optionSelect[#optionSelect][4]
								)
							then
								UiSelectHighlight(
									optionButtons[showSelectOptions][1],
...
function getSliderValue(draggingSlider, mx)
	...
	elseif options[draggingSlider].step ~= nil then
		value = math.floor((value + (options[draggingSlider].step / 2)) / options[draggingSlider].step)
				...
				if release then
						...
						if optionButtons then
							for i, o in pairs(optionButtons) do
								if options[i] then
									...
									elseif
										options[i].type == "slider" and math_isInRect(mx, my, o[1], o[2], o[3], o[4])
									then
									elseif
										options[i].type == "select" and math_isInRect(mx, my, o[1], o[2], o[3], o[4])
									then
									elseif
										optionHover[i]
										and options[i].onclick ~= nil
										and math_isInRect(
											mx,
											my,
											optionHover[i][1],
					...
					if not showSelectOptions then
						for i, o in pairs(optionButtons) do
							...
							elseif
								options[i].type == "slider"
								and (
									math_isInRect(mx, my, o.sliderXpos[1], o[2], o.sliderXpos[2], o[4])
									or math_isInRect(mx, my, o[1], o[2], o[3], o[4])
								)
							then
								draggingSlider = i
...
function GetWidgetToggleValue(widgetname)
	if widgetHandler.orderList[widgetname] == nil or widgetHandler.orderList[widgetname] == 0 then
		return false
	elseif
		widgetHandler.orderList[widgetname] >= 1
		and widgetHandler.knownWidgets ~= nil
		and widgetHandler.knownWidgets[widgetname] ~= nil
	then
...
function loadWidgetData(widgetName, optionId, configVar)
	...
	if
		optionID
		and widgetHandler.configData[widgetName] ~= nil
		and widgetHandler.configData[widgetName][configVar[1]] ~= nil
	then
		if configVar[2] ~= nil and widgetHandler.configData[widgetName][configVar[1]][configVar[2]] ~= nil then
			if
				configVar[3] ~= nil
				and widgetHandler.configData[widgetName][configVar[1]][configVar[2]][configVar[3]] ~= nil
			then
...
function init()
	...
	for index, display in ipairs(displays) do
		if display.width > 0 then
			...
			if
				v_px >= display.x
				and v_px < display.x + display.width
				and v_py >= display.y
				and v_py < display.y + display.height
			then
	...
	options = {
		...
		},
		{
			id = "label_gfx_screen",
			group = "gfx",
			name = BAR.I18N("ui.settings.option.label_screen"),
			category = types.basic,
		},
		...
		},
		{
			...
			type = "select",
			options = {
				BAR.I18N("ui.settings.option.select_off"),
				BAR.I18N("ui.settings.option.select_enabled"),
				BAR.I18N("ui.settings.option.select_adaptive"),
			},
			value = 2,
		...
		{
			id = "limitoffscreenfps",
			group = "gfx",
			category = types.advanced,
			widget = "Limit idle FPS",
			name = BAR.I18N("ui.settings.option.limitoffscreenfps"),
			type = "bool",
			value = GetWidgetToggleValue("Limit idle FPS"),
			description = BAR.I18N("ui.settings.option.limitoffscreenfps_descr"),
		},
		{
			id = "limitidlefps",
			group = "gfx",
...

		{
			...
			onchange = function(i, value)
				if value == 1 then
					...
					Spring.SetConfigInt(
						"MSAALevel",
						tonumber(string.sub(options[getOptionByID("msaa")].options[value], 2))
					)
				end
			end,
		},
...

		{
			...
			onchange = function(i, value)
				...
				saveOptionValue(
					...
					"setSharpness",
					{ "SHARPNESS" },
					options[getOptionByID("cas_sharpness")].value
				)
			end,
		},

		{
			...
			category = types.advanced,
			widget = "Sepia Tone",
			name = BAR.I18N("ui.settings.option.sepiatone"),
			type = "bool",
			value = GetWidgetToggleValue("Sepia Tone"),
		},
		{
			id = "sepiatone_gamma",
			group = "gfx",
		...
		{
			id = "label_gfx_lighting",
			group = "gfx",
			name = BAR.I18N("ui.settings.option.label_lighting"),
			category = types.basic,
		},
...

		{
			...
			category = types.basic,
			widget = "SSAO",
			name = BAR.I18N("ui.settings.option.ssao"),
			type = "bool",
			value = GetWidgetToggleValue("SSAO"),
			description = BAR.I18N("ui.settings.option.ssao_descr"),
		},
		{
			id = "ssao_strength",
			group = "gfx",
...

		{
			...
			group = "gfx",
			category = types.basic,
			widget = "Bloom Shader Deferred",
			name = BAR.I18N("ui.settings.option.bloomdeferred"),
			type = "bool",
			value = GetWidgetToggleValue("Bloom Shader Deferred"),
			description = BAR.I18N("ui.settings.option.bloomdeferred_descr"),
		},
		{
		...
		},
		{
			...
			onload = function(i)
				if
					widgetHandler.configData["Bloom Shader Deferred"] ~= nil
					and widgetHandler.configData["Bloom Shader Deferred"].preset ~= nil
				then
					options[getOptionByID("bloomdeferred_quality")].value =
						widgetHandler.configData["Bloom Shader Deferred"].preset
				end
			end,
			onchange = function(i, value)
...

		{
			...
			type = "bool",
			value = Spring.GetConfigInt("NanoParticlesUpdateLuaUI", 0) == 1,
		...
		{
			id = "distortioneffects",
			group = "gfx",
			category = types.basic,
			widget = "Distortion GL4",
			name = BAR.I18N("ui.settings.option.distortioneffects"),
			type = "bool",
			value = GetWidgetToggleValue("Distortion GL4"),
			description = BAR.I18N("ui.settings.option.distortioneffects_descr"),
		},
...

		{
			...
			group = "gfx",
			name = BAR.I18N("ui.settings.option.label_environment"),
			category = types.basic,
		},
...

		{
			...
			group = "gfx",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.lineofsight") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.losopacity"
			),
			type = "slider",
			min = 0.01,
			max = 1,
...

		{
			...
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.lineofsight") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.fogdiaglines"
			),
			type = "slider",
			min = 0,
			max = 1,
			step = 0.01,
			value = (WG.fogdiaglines ~= nil and WG.fogdiaglines.getStrength ~= nil and WG.fogdiaglines.getStrength())
				or 0.30,
			description = "",
			onload = function(i)
...

		{
			...
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.lineofsight") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.fogdiaglines_blur"
			),
			type = "slider",
			min = 0,
			max = 1,
			...
			value = (
				WG.fogdiaglines ~= nil
				and WG.fogdiaglines.getBlurriness ~= nil
				and WG.fogdiaglines.getBlurriness()
			) or 0.187,
			description = "",
			onload = function(i)
...

		{
			...
			name = BAR.I18N("ui.settings.option.mapedgeextension"),
			type = "bool",
			value = GetWidgetToggleValue("Map Edge Extension"),
			description = BAR.I18N("ui.settings.option.mapedgeextension_descr"),
		},
...

		{
			...
			category = types.basic,
			widget = "Decals GL4",
			name = BAR.I18N("ui.settings.option.decalsgl4"),
			type = "bool",
			value = GetWidgetToggleValue("Decals GL4"),
		},
		{
...

		{
			...
			group = "gfx",
			category = types.basic,
			widget = "Map Grass GL4",
			name = BAR.I18N("ui.settings.option.grass"),
			type = "bool",
			value = GetWidgetToggleValue("Map Grass GL4"),
			description = BAR.I18N("ui.settings.option.grass_desc"),
		},
		{
			id = "grassdistance",
			group = "gfx",
...

		{
			...
			category = types.basic,
			widget = "Snow",
			name = BAR.I18N("ui.settings.option.snow"),
			type = "bool",
			value = GetWidgetToggleValue("Snow"),
			description = BAR.I18N("ui.settings.option.snow_descr"),
		},
		{
			id = "snowmap",
			group = "gfx",
...

		{
			...
			group = "gfx",
			category = types.advanced,
			widget = "Volumetric Clouds",
			name = BAR.I18N("ui.settings.option.clouds"),
			type = "bool",
			value = GetWidgetToggleValue("Volumetric Clouds"),
			description = "",
		},
		{
			id = "clouds_opacity",
		...
		{
			id = "label_gfx_effects",
			group = "gfx",
			name = BAR.I18N("ui.settings.option.label_effects"),
			category = types.basic,
		},
			...
			onchange = function(i, value)
				...
				if Spring.GetConfigInt("NanoParticleMode", 1) == 0 then
					...
					if Spring.GetConfigInt("NanoParticleMode", 1) == 0 then
						Spring.SetConfigInt(
							"MaxNanoParticles",
							math.floor(Spring.GetConfigInt("MaxParticles", 15000) * 0.34)
						)
					else
...

		{
			...
			name = BAR.I18N("ui.settings.option.dof"),
			type = "bool",
			value = GetWidgetToggleValue("Depth of Field"),
			description = BAR.I18N("ui.settings.option.dof_descr"),
		},
		{
			id = "dof_autofocus",
			group = "gfx",
...

		{
			...
			group = "sound",
			category = types.basic,
			name = BAR.I18N("ui.settings.option.volume") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.sndvolmaster"
			),
			type = "slider",
			min = 0,
			max = 80,
...

		{
			...
			group = "sound",
			category = types.basic,
			name = BAR.I18N("ui.settings.option.label_soundtrack"),
		},
...

		{
			...
			group = "notif",
			name = BAR.I18N("ui.settings.option.label_messages"),
			category = types.basic,
		},
		...
		-- CONTROL
		{
			id = "label_ui_hotkeys",
			group = "control",
			name = BAR.I18N("ui.settings.option.label_hotkeys"),
			category = types.basic,
		},
		...
		},
		{
			...
			value = (
				WG.gridmenu ~= nil
				and WG.gridmenu.getShiftKeyModifier ~= nil
				and WG.gridmenu.getShiftKeyModifier()
			),
...

		{
			id = "label_ui_cursor",
			group = "control",
			name = BAR.I18N("ui.settings.option.label_cursor"),
			category = types.basic,
		},
...

		{
			id = "label_ui_camera",
			group = "control",
			name = BAR.I18N("ui.settings.option.label_camera"),
			category = types.basic,
		},
...

		{
			...
			type = "select",
			options = {
				BAR.I18N("ui.settings.option.select_firstperson"),
				BAR.I18N("ui.settings.option.select_overhead"),
				BAR.I18N("ui.settings.option.select_springcam"),
				BAR.I18N("ui.settings.option.select_rotoverhead"),
				BAR.I18N("ui.settings.option.select_free"),
			},
		...
		},
		{
			...
			type = "select",
			options = {
				BAR.I18N("ui.settings.option.select_constant"),
		...
		--},
		{
			...
			type = "select",
			options = {
				BAR.I18N("ui.settings.option.smoothing_exponential"),
				BAR.I18N("ui.settings.option.smoothing_spring"),
			},
		...
		{
			id = "scrolltoggleoverview",
			group = "control",
			category = types.advanced,
			widget = "Scrolldown Toggleoverview",
			name = widgetOptionColor .. "   " .. BAR.I18N("ui.settings.option.scrolltoggleoverview"),
			type = "bool",
			value = GetWidgetToggleValue("Scrolldown Toggleoverview"),
			description = BAR.I18N("ui.settings.option.scrolltoggleoverview_descr"),
		},

		{
			...
			group = "control",
			category = types.advanced,
			widget = "Overview Camera Keep Position",
			name = widgetOptionColor .. "   " .. BAR.I18N("ui.settings.option.camoverviewrestore"),
			type = "bool",
			value = GetWidgetToggleValue("Overview Camera Keep Position"),
			description = BAR.I18N("ui.settings.option.camoverviewrestore_descr"),
		},

		{
			...
			group = "control",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.lockcamera") .. widgetOptionColor .. "   " .. BAR.I18N(
				"ui.settings.option.lockcamera_transitiontime"
			),
			type = "slider",
			min = 0.5,
			max = 1.7,
...

		{
			id = "label_ui_command",
			group = "control",
			name = BAR.I18N("ui.settings.option.label_commands"),
			category = types.advanced,
		},
		...
		-- INTERFACE
		{
			id = "label_ui_interface",
			group = "ui",
			name = BAR.I18N("ui.settings.option.label_interface"),
			category = types.basic,
		},
		...
		},
		{
			...
			group = "ui",
			category = types.basic,
			name = BAR.I18N("ui.settings.option.interface") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.uiscale"
			),
			type = "slider",
			min = 0.8,
			max = 1.3,
...

		{
			...
			category = types.basic,
			widget = "GUI Shader",
			name = widgetOptionColor .. "   " .. BAR.I18N("ui.settings.option.guishader"),
			type = "bool",
			value = GetWidgetToggleValue("GUI Shader"),
		},

		{
			...
			group = "ui",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.minimap") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.minimap_maxheight"
			),
			type = "slider",
			min = 0.2,
			max = 0.4,
		...
		},
		{
			...
			type = "select",
			options = {
				BAR.I18N("ui.settings.option.minimaprotation_none"),
				BAR.I18N("ui.settings.option.minimaprotation_autoflip"),
				BAR.I18N("ui.settings.option.minimaprotation_autorotate"),
				BAR.I18N("ui.settings.option.minimaprotation_autolandscape"),
			},
		...
		},
		{
			...
			group = "ui",
			category = types.advanced,
			widget = "Picture-in-Picture Minimap",
			name = widgetOptionColor .. "   " .. BAR.I18N("ui.settings.option.minimappip"),
			type = "bool",
			value = GetWidgetToggleValue("Picture-in-Picture Minimap"),
			description = BAR.I18N("ui.settings.option.minimappip_descr"),
		},
		{
			id = "pip_altkeyzoom",
			group = "ui",
		...
		-- },
		{
			...
			onload = function(i)
				if WG.minimap and WG.minimap.getEngineMinimapFallbackThreshold then
					options[getOptionByID("pip_engine_fallback_threshold")].value =
...

		{
			...
			name = BAR.I18N("ui.settings.option.pip"),
			type = "bool",
			value = GetWidgetToggleValue("Picture-in-Picture"),
			description = BAR.I18N("ui.settings.option.pip_descr"),
		},
		{
			id = "pip2",
			group = "ui",
			category = types.advanced,
			widget = "Picture-in-Picture 2",
			name = BAR.I18N("ui.settings.option.pip2"),
			type = "bool",
			value = GetWidgetToggleValue("Picture-in-Picture 2"),
			description = BAR.I18N("ui.settings.option.pip2_descr"),
		},

		{
			...
			group = "ui",
			category = types.basic,
			name = BAR.I18N("ui.settings.option.buildmenu") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.buildmenu_bottom"
			),
			type = "bool",
			value = (
				WG.buildmenu ~= nil
				and WG.buildmenu.getBottomPosition ~= nil
				and WG.buildmenu.getBottomPosition()
			),
...

		{
			...
			group = "ui",
			category = types.basic,
			name = BAR.I18N("ui.settings.option.ordermenu") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.ordermenu_bottompos"
			),
			type = "bool",
			value = (
				WG.ordermenu ~= nil
				and WG.ordermenu.getBottomPosition ~= nil
				and WG.ordermenu.getBottomPosition()
			),
		...
		},
		{
			...
			value = (
				WG.ordermenu ~= nil
				and WG.ordermenu.getDisabledCmd ~= nil
				and WG.ordermenu.getDisabledCmd("Move")
			),
...

		{
			...
			group = "ui",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.info") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.info_buildlist"
			),
			type = "bool",
...

		{
			...
			group = "ui",
			category = types.basic,
			name = BAR.I18N("ui.settings.option.advplayerlist") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.advplayerlist_country"
			),
			type = "bool",
			value = true,
		...
		},
		{
			...
			group = "ui",
			category = types.advanced,
			widget = "AdvPlayersList Unit Totals",
			name = widgetOptionColor .. "   " .. BAR.I18N("ui.settings.option.unittotals"),
			type = "bool",
			value = GetWidgetToggleValue("AdvPlayersList Unit Totals"),
			description = BAR.I18N("ui.settings.option.unittotals_descr"),
		},
		{
			id = "musicplayer",
			group = "ui",
		...
		},
		{
			...
			group = "ui",
			category = types.advanced,
			widget = "AdvPlayersList Mascot",
			name = widgetOptionColor .. "   " .. BAR.I18N("ui.settings.option.mascot"),
			type = "bool",
			value = GetWidgetToggleValue("AdvPlayersList Mascot"),
			description = BAR.I18N("ui.settings.option.mascot_descr"),
		},
...

		{
			...
			group = "ui",
			category = types.basic,
			name = BAR.I18N("ui.settings.option.console") .. "   " .. widgetOptionColor .. BAR.I18N(
				"ui.settings.option.console_fontsize"
			),
			type = "slider",
			min = 0.92,
			max = 1.12,
		...
		},
		{
			...
			group = "ui",
			category = types.basic,
			name = widgetOptionColor .. "      " .. widgetOptionColor .. BAR.I18N(
				"ui.settings.option.console_hidespecchatplayer"
			),
			type = "bool",
		...
		--},
		{
			...
			name = BAR.I18N("ui.settings.option.autoeraser"),
			type = "bool",
			value = GetWidgetToggleValue("Auto mapmark eraser"),
			description = BAR.I18N("ui.settings.option.autoeraser_descr"),
		},
		{
...

		{
			...
			group = "ui",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.topbar") .. widgetOptionColor .. "  " .. BAR.I18N(
...

		{
			...
			category = types.basic,
			widget = "Unit Groups",
			name = BAR.I18N("ui.settings.option.unitgroups"),
			type = "bool",
			value = GetWidgetToggleValue("Unit Groups"),
			description = BAR.I18N("ui.settings.option.unitgroups_descr"),
		},
		{
			id = "idlebuilders",
			group = "ui",
			...
			name = BAR.I18N("ui.settings.option.idlebuilders"),
			type = "bool",
			value = GetWidgetToggleValue("Idle Builders"),
			description = BAR.I18N("ui.settings.option.idlebuilders_descr"),
		},
		{
			id = "buildbar",
			group = "ui",
			category = types.basic,
			widget = "BuildBar",
			name = BAR.I18N("ui.settings.option.buildbar"),
			type = "bool",
			value = GetWidgetToggleValue("BuildBar"),
			description = BAR.I18N("ui.settings.option.buildbar_descr"),
		},

		{
			...
			name = BAR.I18N("ui.settings.option.converterusage"),
			type = "bool",
			value = GetWidgetToggleValue("Converter Usage"),
			description = BAR.I18N("ui.settings.option.converterusage_descr"),
		},
...

		{
			id = "label_ui_visuals",
			group = "ui",
			name = BAR.I18N("ui.settings.option.label_visuals"),
			category = types.basic,
		},
...

		{
			...
			category = types.basic,
			widget = "TeamPlatter",
			name = BAR.I18N("ui.settings.option.teamplatter"),
			type = "bool",
			value = GetWidgetToggleValue("TeamPlatter"),
			description = BAR.I18N("ui.settings.option.teamplatter_descr"),
		},
		{
...

		{
			...
			category = types.basic,
			widget = "EnemySpotter",
			name = BAR.I18N("ui.settings.option.enemyspotter"),
			type = "bool",
			value = GetWidgetToggleValue("EnemySpotter"),
			description = BAR.I18N("ui.settings.option.enemyspotter_descr"),
		},
		{
		...
		--{ id = "selectedunits", group = "ui", category = types.basic, widget = "Selected Units GL4", name = "Selection", type = "bool", value = GetWidgetToggleValue("Selected Units GL4"), description = Spring.I18N('ui.settings.option.selectedunits_descr') },
		{
			...
			group = "ui",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.selectedunits") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.selectedunits_opacity"
			),
			min = 0,
			max = 0.5,
			step = 0.01,
...

		{
			...
			group = "ui",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.ghosticons") .. widgetOptionColor .. "   " .. BAR.I18N(
				"ui.settings.option.ghosticons_brightness"
			),
			min = 0,
			max = 1.0,
			step = 0.15,
...

		{
			id = "label_ui_info",
			group = "ui",
			name = BAR.I18N("ui.settings.option.label_info"),
			category = types.basic,
		},
...

		{
			...
			group = "ui",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.metalspots") .. widgetOptionColor .. "   " .. BAR.I18N(
				"ui.settings.option.metalspots_values"
			),
			type = "bool",
			...
			onchange = function(i, value)
				...
				saveOptionValue(
					...
					"setShowValue",
					{ "showValue" },
					options[getOptionByID("metalspots_values")].value
				)
			end,
		},
		{
			...
			onchange = function(i, value)
				saveOptionValue(
					...
					"setMetalViewOnly",
					{ "showValue" },
					options[getOptionByID("metalspots_metalviewonly")].value
				)
			end,
		},

		{
			...
			name = BAR.I18N("ui.settings.option.geospots"),
			type = "bool",
			value = GetWidgetToggleValue("Metalspots"),
			description = BAR.I18N("ui.settings.option.geospots_descr"),
		},

		{
			...
			group = "ui",
			category = types.advanced,
			name = BAR.I18N("ui.settings.option.healthbars") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.healthbarsscale"
			),
			type = "slider",
			min = 0.6,
			max = 2.0,
		...
		},
		{
			...
			name = BAR.I18N("ui.settings.option.rankicons"),
			type = "bool",
			value = GetWidgetToggleValue("Rank Icons GL4"),
			description = BAR.I18N("ui.settings.option.rankicons_descr"),
		},
		{
...

		{
			...
			category = types.basic,
			widget = "AllyCursors",
			name = BAR.I18N("ui.settings.option.allycursors"),
			type = "bool",
			value = GetWidgetToggleValue("AllyCursors"),
			description = BAR.I18N("ui.settings.option.allycursors_descr"),
		},
		{
		...
		},
		{
			...
			onchange = function(i, value)
				saveOptionValue(
					...
					"setLights",
					{ "addLights" },
					options[getOptionByID("allycursors_lights")].value
				)
			end,
		},
		{
...

		{
			...
			name = BAR.I18N("ui.settings.option.showbuilderqueue"),
			type = "bool",
			value = GetWidgetToggleValue("Show Builder Queue"),
			description = BAR.I18N("ui.settings.option.showbuilderqueue_descr"),
		},

		{
			...
			name = BAR.I18N("ui.settings.option.unitenergyicons"),
			type = "bool",
			value = GetWidgetToggleValue("Unit Energy Icons"),
			description = BAR.I18N("ui.settings.option.unitenergyicons_descr"),
		},

		{
			id = "unitidlebuildericons",
			group = "ui",
			category = types.advanced,
			widget = "Unit Idle Builder Icons",
			name = BAR.I18N("ui.settings.option.unitidlebuildericons"),
			type = "bool",
			value = GetWidgetToggleValue("Unit Idle Builder Icons"),
			description = BAR.I18N("ui.settings.option.unitidlebuildericons_descr"),
		},

		{
			id = "unitfirestateicons",
			group = "ui",
			category = types.advanced,
			widget = "Unit Fire State Icons",
			name = BAR.I18N("ui.settings.option.unitfirestateicons"),
			type = "bool",
			value = GetWidgetToggleValue("Unit Fire State Icons"),
			description = BAR.I18N("ui.settings.option.unitfirestateicons_descr"),
		},

		{
			id = "resurrectionhalos",
			group = "ui",
			category = types.advanced,
			widget = "Resurrection Halos GL4",
			name = BAR.I18N("ui.settings.option.resurrectionhalos"),
			type = "bool",
			value = GetWidgetToggleValue("Resurrection Halos GL4"),
			description = BAR.I18N("ui.settings.option.resurrectionhalos_descr"),
		},
...

		{
			...
			category = types.basic,
			widget = "Commands FX",
			name = BAR.I18N("ui.settings.option.commandsfx"),
			type = "bool",
			value = GetWidgetToggleValue("Commands FX"),
			description = BAR.I18N("ui.settings.option.commandsfx_descr"),
		},
		...
		},
		{
			...
			onchange = function(i, value)
				saveOptionValue(
					"Commands FX",
					"commandsfx",
					"setUseTeamColorsWhenSpec",
					{ "useTeamColorsWhenSpec" },
					value
				)
			end,
		},

		{
			...
			name = BAR.I18N("ui.settings.option.flankingicons"),
			type = "bool",
			value = GetWidgetToggleValue("Flanking Icons GL4"),
			description = BAR.I18N("ui.settings.option.flankingicons_descr"),
		},
...

		{
			...
			category = types.advanced,
			widget = "Given Units",
			name = BAR.I18N("ui.settings.option.givenunits"),
			type = "bool",
			value = GetWidgetToggleValue("Given Units"),
			description = BAR.I18N("ui.settings.option.givenunits_descr"),
		},

		{
			id = "reclaimfieldhighlight",
			group = "ui",
			category = types.advanced,
			widget = "Reclaim Field Highlight",
			name = BAR.I18N("ui.settings.option.reclaimfieldhighlight"),
			type = "bool",
			description = BAR.I18N("ui.settings.option.reclaimfieldhighlight_descr"),
		},
		...
		{
			id = "highlightcomwrecks",
			group = "ui",
			category = types.advanced,
			widget = "Highlight Commander Wrecks",
			name = BAR.I18N("ui.settings.option.highlightcomwrecks"),
			type = "bool",
			value = GetWidgetToggleValue("Highlight Commander Wrecks"),
			description = BAR.I18N("ui.settings.option.highlightcomwrecks_descr"),
		},
		{
...

		{
			...
			name = BAR.I18N("ui.settings.option.buildinggrid"),
			type = "bool",
			value = GetWidgetToggleValue("Building Grid GL4"),
			description = BAR.I18N("ui.settings.option.buildinggrid_descr"),
		},
		{
			...
			max = 1,
			step = 0.05,
			value = (WG.buildinggrid ~= nil and WG.buildinggrid.getOpacity ~= nil and WG.buildinggrid.getOpacity())
				or 1,
			description = "",
			onload = function(i)
			...
			onchange = function(i, value)
				if
					widgetHandler.orderList["Building Grid GL4"]
					and widgetHandler.orderList["Building Grid GL4"] >= 0.5
				then
		...
		{
			id = "startpositionsuggestions",
			group = "ui",
			category = types.basic,
			widget = "Start Position Suggestions",
			name = BAR.I18N("ui.settings.option.startpositionsuggestions"),
			type = "bool",
			value = GetWidgetToggleValue("Start Position Suggestions"),
			description = BAR.I18N("ui.settings.option.startpositionsuggestions_descr"),
		},

		{
			id = "label_ui_ranges",
			group = "ui",
			name = BAR.I18N("ui.settings.option.label_ranges"),
			category = types.basic,
		},
		...
		-- Radar range rings:
		{
			...
			name = BAR.I18N("ui.settings.option.radarrange"),
			type = "bool",
			value = GetWidgetToggleValue("Sensor Ranges Radar"),
			description = BAR.I18N("ui.settings.option.radarrange_descr"),
		},
		...
		-- Sonar range
		{
			...
			name = BAR.I18N("ui.settings.option.sonarrange"),
			type = "bool",
			value = GetWidgetToggleValue("Sensor Ranges Sonar"),
			description = BAR.I18N("ui.settings.option.sonarrange_descr"),
		},
		...
		-- Jammer range
		{
			...
			name = BAR.I18N("ui.settings.option.jammerrange"),
			type = "bool",
			value = GetWidgetToggleValue("Sensor Ranges Jammer"),
			description = BAR.I18N("ui.settings.option.jammerrange_descr"),
		},

		{
			...
			max = 0.66,
			step = 0.01,
			value = (WG.jammerrange ~= nil and WG.jammerrange.getOpacity ~= nil and WG.jammerrange.getOpacity())
				or 0.08,
			description = "",
			onload = function(i)
		...
		-- LOS Range:
		{
			...
			name = BAR.I18N("ui.settings.option.losrange"),
			type = "bool",
			value = GetWidgetToggleValue("Sensor Ranges LOS"),
			description = BAR.I18N("ui.settings.option.losrange_descr"),
		},
...

		{
			...
			name = BAR.I18N("ui.settings.option.attackrange"),
			type = "bool",
			value = GetWidgetToggleValue("Attack Range GL4"),
			description = BAR.I18N("ui.settings.option.attackrange_descr"),
		},
		{
		...
		},
		{
			...
			value = (
				WG.attackrange ~= nil
				and WG.attackrange.getCursorUnitRange ~= nil
				and WG.attackrange.getCursorUnitRange()
			),
		...
		},
		{
			...
			max = 1,
			step = 0.1,
			value = (WG.attackrange ~= nil and WG.attackrange.getOpacity ~= nil and WG.attackrange.getNumRangesMult())
				or 1,
...

		{
			...
			name = BAR.I18N("ui.settings.option.defrange"),
			type = "bool",
			value = GetWidgetToggleValue("Defense Range GL4"),
			description = BAR.I18N("ui.settings.option.defrange_descr"),
		},
...

		{
			...
			category = types.advanced,
			widget = "Anti Ranges",
			name = BAR.I18N("ui.settings.option.antiranges"),
			type = "bool",
			value = GetWidgetToggleValue("Anti Ranges"),
			description = BAR.I18N("ui.settings.option.antiranges_descr"),
		},

		{
			id = "label_ui_spectator",
			group = "ui",
			name = BAR.I18N("ui.settings.option.label_spectator"),
			category = types.basic,
		},
...

		{
			...
			name = BAR.I18N("ui.settings.option.spectator_hud"),
			type = "bool",
			value = GetWidgetToggleValue("Spectator HUD"),
			description = BAR.I18N("ui.settings.option.spectator_hud_descr"),
		},
		{
			...
			step = 0.1,
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getWidgetSize ~= nil
				and WG.spectator_hud.getWidgetSize()
			) or 0.8,
			description = "",
			onload = function(i)
...

		{
			...
			value = (WG.spectator_hud ~= nil and WG.spectator_hud.getConfig ~= nil and WG.spectator_hud.getConfig())
				or 1,
...

		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("metalIncome")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("energyIncome")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("buildPower")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("metalProduced")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("energyProduced")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("metalExcess")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("energyExcess")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("armyValue")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("defenseValue")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("utilityValue")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("economyValue")
			) or 1,
		...
		},
		{
			...
			value = (
				WG.spectator_hud ~= nil
				and WG.spectator_hud.getMetricEnabled ~= nil
				and WG.spectator_hud.getMetricEnabled("damageDealt")
			) or 1,
...

		{
			...
			group = "ui",
			name = BAR.I18N("ui.settings.option.label_developer"),
			category = types.advanced,
		},
		...
		},
		{
			...
			category = types.basic,
			widget = "Autoquit",
			name = BAR.I18N("ui.settings.option.autoquit"),
			type = "bool",
			value = GetWidgetToggleValue("Autoquit"),
			description = BAR.I18N("ui.settings.option.autoquit_descr"),
		},
		...
		{
			id = "label_ui_behavior",
			group = "game",
			name = BAR.I18N("ui.settings.option.label_behavior"),
			category = types.basic,
		},
...

		{
			...
			name = BAR.I18N("ui.settings.option.prioconturrets"),
			type = "bool",
			value = GetWidgetToggleValue("Priority Construction Turrets"),
...

		{
			id = "label_teamcolors",
			group = "accessibility",
			name = BAR.I18N("ui.settings.option.label_teamcolors"),
			category = types.basic,
		},
...

		{
			...
			name = BAR.I18N("ui.settings.option.autocheat"),
			type = "bool",
			value = GetWidgetToggleValue("Dev Auto cheat"),
			description = BAR.I18N("ui.settings.option.autocheat_descr"),
		},
		{
			id = "restart",
			group = "dev",
...

		{
			...
			name = BAR.I18N("ui.settings.option.profiler") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.profiler_widget"
			),
			type = "bool",
			value = GetWidgetToggleValue("Widget Profiler"),
		},
		{
			id = "profiler_gadget",
			group = "dev",
		...
		},
		{
			...
			name = BAR.I18N("ui.settings.option.framegrapher"),
			type = "bool",
			value = GetWidgetToggleValue("Frame Grapher"),
			description = "",
		},
...

		{
			id = "label_dev_other",
			group = "dev",
			name = BAR.I18N("ui.settings.option.label_other"),
			category = types.dev,
		},
...

		{
			...
			group = "dev",
			category = types.dev,
			widget = "CEG Effects Browser",
			name = BAR.I18N("ui.settings.option.cegbrowser"),
			type = "bool",
			value = GetWidgetToggleValue("CEG Effects Browser"),
			description = BAR.I18N("ui.settings.option.cegbrowser_descr"),
		},

		{
			...
			name = BAR.I18N("ui.settings.option.startboxeditor"),
			type = "bool",
			value = GetWidgetToggleValue("Startbox Editor"),
			description = BAR.I18N("ui.settings.option.startboxeditor_descr"),
		},
...

		{
			id = "label_dev_map",
			group = "dev",
			name = BAR.I18N("ui.settings.option.label_map"),
			category = types.dev,
		},
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.sun") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.sun_y"
			),
			type = "slider",
			min = 0.05,
			max = 0.9999,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.fog") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.fog_start"
			),
			type = "slider",
			min = 0,
			max = 1.99,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.color_groundambient") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.red"
			),
			type = "slider",
			min = 0,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.color_grounddiffuse") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.red"
			),
			type = "slider",
			min = 0,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.color_groundspecular") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.red"
			),
			type = "slider",
			min = 0,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.color_unitambient") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.red"
			),
			type = "slider",
			min = 0,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.color_unitdiffuse") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.red"
			),
			type = "slider",
			min = 0,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.color_unitspecular") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.red"
			),
			type = "slider",
			min = 0,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.skycolor") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.red"
			),
			type = "slider",
			min = 0,
...

		{
			...
			group = "dev",
			category = types.dev,
			name = BAR.I18N("ui.settings.option.skybox") .. widgetOptionColor .. "  " .. BAR.I18N(
				"ui.settings.option.angle"
			),
			type = "slider",
			min = -3.14,
			max = 3.14,
		...
		},
		{
			id = "label_dev_water",
			group = "dev",
			name = BAR.I18N("ui.settings.option.label_water"),
			category = types.dev,
		},
		...
		},
		{
			...
			description = "The initial frequency of the bump map repetition rate. Larger numbers mean more tiles",
			onload = function(i) end,
			onchange = function(i, value)
		...
		},
		{
			...
			description = "How much smaller each additional repetition of the normal map should be. Larger numbers mean smaller",
			onload = function(i) end,
			onchange = function(i, value)
		...
		},
		{
			...
			description = "How strong each additional repetition of the normal map should be",
			onload = function(i) end,
			onchange = function(i, value)
	...
	if getOptionByID("font") then
		...
		for k, file in ipairs(files) do
			...
			if ext == "otf" or ext == "ttf" or ext == "ttc" then
				...
				if not fontsn[name:lower()] then
					...
					fontOption[#fonts] =
						gl.LoadFont("fonts/" .. fontsFull[#fontsFull], 20 * fontScale, 5 * fontScale, 1.5)
				end
			end
		end
		...
		options[getOptionByID("font")].value =
			getSelectKey(getOptionByID("font"), string.sub(fname, 1, string.len(fname) - 4))
		...
		options[getOptionByID("font2")].value =
			getSelectKey(getOptionByID("font2"), string.sub(fname, 1, string.len(fname) - 4))
	end
	...
	if getOptionByID("shadows_opacity") then
		if gl.GetSun("shadowDensity") < options[getOptionByID("shadows_opacity")].min then
			Spring.SetSunLighting({
				groundShadowDensity = options[getOptionByID("shadows_opacity")].min,
				modelShadowDensity = options[getOptionByID("shadows_opacity")].min,
			})
		end
	end
	...
	if type(trackList) == "table" then
		...
		for i, option in pairs(options) do
			...
			if option.id == "soundtrackFades" then
				count = count + 1
				newOptions[count] = {
					id = "label_sound_music",
					group = "sound",
					name = BAR.I18N("ui.settings.option.label_playlist"),
					category = types.basic,
				}
				count = count + 1
				...
				for k, v in pairs(trackList) do
					if prevCategory ~= v[1] then
						...
						count = count + 1
						newOptions[count] =
							{ id = "music_track_" .. v[2], group = "sound", basic = true, name = v[1], type = "text" }
					end
					count = count + 1
					newOptions[count] = {
	...
	elseif
		widgetHandler.configData.Notifications ~= nil
		and widgetHandler.configData.Notifications.notificationList ~= nil
	then
	...
	for name, data in pairs(widgetHandler.knownWidgets) do
		if not data.fromZip then
			if not userwidgetsDetected then
				...
				options[#options + 1] = {
					id = "label_custom_widgets",
					group = "custom",
					name = BAR.I18N("ui.settings.option.label_widgets"),
					category = types.basic,
				}
			...
			if data.author and data.author ~= "" then
				desc = desc
					.. (desc ~= "" and "\n" or "")
					.. widgetOptionColor
					.. BAR.I18N("ui.settings.option.author")
					.. ": "
					.. data.author
			end
			options[#options + 1] = {
	...
	if customOptionsCount > 0 then
		options[#options + 1] = {
			id = "label_custom_options",
			group = "custom",
			name = BAR.I18N("ui.settings.option.label_options"),
			category = types.basic,
		}
...
function widget:Initialize()
	...
	if
		...
		and (Spring.GetConfigInt("CamMode", 2) == 2 or Spring.GetConfigInt("CamMode", 2) == 3)
	then
