--- variables.lua's authoring surface: typed, document-declared slots. Pure
--- Lua; the value lives in the pile (ctx.GetVariable/SetVariable), so a
--- variable is save-load proof and never a global.
...
function Variables.ForFile(filename)
	...
	local Finalize = function(exports)
		...
		-- Anything may export; the handles are the point, but a default or a
		-- threshold shared with the trigger files is a value like any other.
		assert(
