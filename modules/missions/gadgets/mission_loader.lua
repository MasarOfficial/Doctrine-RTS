...
-- What a contribution's Context is built over: the roster and the ledgers
-- the loader owns, never its tables. A module reaches a unit by roster
-- name and holds its protections through the ledger that survives a reload.
---@type MissionRuntime
local runtime = {
...
local function loadMission(missionName, preserve)
	...
	-- A contribution brings the context its verbs act through and the events
	-- its module raises, exactly as it brings its vocabulary: the loader
	-- composes, it does not know what waves or combat are. What a trigger may
	-- listen for is a callin or a registered event — anything else is a
	-- typo, and a typo is a load error, not a condition that never wakes.
