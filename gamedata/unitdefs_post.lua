...
--------------------------------------------------------------------------------
-- DEV/DEBUG: global speed multipliers, active only when cheats are enabled.
-- Applies to unit defs at load time: movement speed, buildpower, buildtime,
-- and health/sight tweaks. Multipliers live in one place below.
--------------------------------------------------------------------------------
if Spring.GetModOptions().dev_fast_mode then
	local SPEED_MULT = 5.0 -- units move 5x faster
	local BUILDPOWER_MULT = 8.0 -- nano turrets/engineers build 8x faster
	local BUILDTIME_DIV = 8.0 -- everything takes 1/8 time to construct

	for _, unitDef in pairs(UnitDefs) do
		if unitDef.speed and unitDef.speed > 0 then
			unitDef.speed = unitDef.speed * SPEED_MULT
		end
		if unitDef.buildspeed and unitDef.buildspeed > 0 then
			unitDef.buildspeed = unitDef.buildspeed * BUILDPOWER_MULT
		end
		if unitDef.workertime and unitDef.workertime > 0 then
			unitDef.workertime = unitDef.workertime * BUILDPOWER_MULT
		end
		if unitDef.buildtime and unitDef.buildtime > 1 then
			unitDef.buildtime = unitDef.buildtime / BUILDTIME_DIV
		end
	end
	Spring.Echo("[DRTS DEBUG] Unit speed/build multipliers applied (cheats on)")
end
